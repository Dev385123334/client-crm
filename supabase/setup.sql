...
-- 7. Sheet connections table (global — one shared connection, visible to all users)
...
create policy "all_users_select_sheets"
...
create policy "all_users_insert_sheets"
  on sheet_connections
  for insert
  to authenticated
  with check (true);
...
create policy "all_users_update_sheets"
  on sheet_connections
  for update
  to authenticated
  using (true)
...
create policy "all_users_delete_sheets"
  on sheet_connections
  for delete
  to authenticated
  using (true);
